...
void Player::issueOrder(Map *theMap, vector<Player *> *thePlayers) {
    ...
    while(deployedAll == false) {
        for (int j = 0; j < defendList->size(); j++) {
            if (usableArmies == 0) {deployedAll = true;}
                ...
                cout << "How many armies will you deploy to " << defendList->at(j)->getTerritoryName() << "?" << endl;
                int amount;
                bool valid = false;
                while(valid == false){
                    cin >> amount;
                    if(amount <= usableArmies && amount >= 0){
                        ...
                        if(amount > 0) {
                            ...
                            cout << amount << " armies will be deployed to" << defendList->at(j)->getTerritoryName()
                                 << "." << endl;
        ...
        for (int i = 0; i < whatWasChosen->size(); i++) {
            ...
            while (chosen == false) {
                ...
                if (ans < 0 || ans > neighboursYouOwn->size()) {
                    ...
                    chosen = true;
                    chosenOne = neighboursYouOwn->at(ans);
                    cout << "Great! You will advance from " << neighboursYouOwn->at(ans)->getTerritoryName() << endl;
                }
            }//end of while (chose proper number)
            int availableArmies = chosenOne->getMockArmies();
    ...
    //Reset mock army count for all territories in defend list:
    for (int i = 0; i < defendList->size(); i++) {
        defendList->at(i)->getMockArmies();
    }//end of for
