...
namespace graph {
    ...
    public:
         ...
         */
        const std::set<V> get_children(const V &vertex) const {
            std::set<V> children;
            if (_edges.find(vertex) == _edges.end()) {
                return children;
            }
         ...
         */
        const std::set<V> get_parents(const V &vertex) const {
            std::set<V> parents;
            if (_edges.find(vertex) == _edges.end()) {
                return parents;
            }
         ...
         */
        const std::set<V> get_neighbours(const V &vertex) const {
            std::set<V> neighbours;
            if (_edges.find(vertex) == _edges.end()) {
                return neighbours;
            }
         ...
         */
        const std::set<V> get_descendants(const V &vertex) const {
            std::set<V> descendents;
            if (_edges.find(vertex) == _edges.end()) {
                return descendents;
            }
            std::set<V> visited;
         ...
         */
        const std::set<V> get_ancestors(const V &vertex) const {
            std::set<V> ancestors;
            if (_edges.find(vertex) == _edges.end()) {
                return ancestors;
            }
        ...
        /**
         * Get every single vertex in the graph as a vector
         */
        std::vector<V> get_vertices() const {
            std::vector<V> vertices;
            for (std::pair<V, Vertex> element : _edges) {
